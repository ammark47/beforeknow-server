...
exports.handler = async (event, context) => {
    ...
    console.log("getting chat currency")
    try {
        const res = await getChatCurrency(parseInt(userId))
...
const getChatCurrency = async (userId) => {
    return await db.one('SELECT chat_currency FROM users WHERE id = $1', [userId])
}